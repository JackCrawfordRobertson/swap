...
const ResultsContent = () => {
    ...
    // Check client-side rendering to avoid hydration issues
    useEffect(() => {
        setIsClient(true);
    }, []);

    // Fetch venues and cities
    useEffect(() => {
        const fetchVenues = async () => {
            ...
            const venuesWithCities = await Promise.all(
                allVenues.map(async (venue) => ({
                    ...venue,
                    city: await fetchCityFromAddress(venue.location),
                }))
            );
    ...
    useEffect(() => {
        if (filteredVenues.length > 0) {
            const filtered = filteredVenues.filter(
                (venue) =>
